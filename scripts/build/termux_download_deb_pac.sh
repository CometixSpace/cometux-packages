...
termux_download_deb_pac() {
	...
	for idx in $(seq ${#TERMUX_REPO_URL[@]}); do
		...
		if [ "${PACKAGE_ARCH}" = 'all' ]; then
			# 'all' comes first for repos that keep a real binary-all list
			# (termux-apt-repo does; aptly mirrors fold those packages into
			# each architecture instead, where the rest of this list finds
			# them).
